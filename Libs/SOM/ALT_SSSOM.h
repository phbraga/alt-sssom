...
class ALT_SSSOM : public SOM<ALT_SSSOMNode> {
public:
    uint maxNodeNumber;
    int epochs;
    float minwd;
...
//        return sqrt(distance);
    ...
    ALT_SSSOM& updateConnections(TNode *node) {
    ...
    ALT_SSSOM& updateAllConnections() {
...
//        enumerateNodes();
    ...
    ALT_SSSOM& trainningStep(int row,  std::vector<int> groups, std::map<int, int> &groupLabels) {
        TVector v(data.cols());
    ...
    ALT_SSSOM& finishMapFixed(bool sorted, std::vector<int> groups, std::map<int, int> &groupLabels) {
    ...
    ALT_SSSOM& updateMap(const TVector &w) {
...
//                unsup_else++;
    ...
    ALT_SSSOM& updateMapSup(const TVector& w, int cls) {
        using namespace std;
...
//            sup_handle_else++;
    ...
    int getNodeIndex(ALT_SSSOMNode &node) {
        SOM<ALT_SSSOMNode>::TPNodeSet::iterator it = meshNodeSet.begin();
        int i = 0;
